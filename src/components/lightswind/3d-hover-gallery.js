...
// Assuming cn is a utility for combining class names, similar to 'clsx' or 'classnames'
// If you don't have this utility, you can replace cn(...) with a simple template literal for class names.
// For example: `class1 class2 ${condition ? 'class3' : ''}`
...
enableKeyboardNavigation = true, autoPlay = false, autoPlayDelay = 3000, className, style, onImageClick, onImageHover, onImageFocus }) => {
    ...
    return (_jsx("div", { 
        // Main container for the gallery, ensuring it takes full width and height
        // and hides any overflow to prevent horizontal scrolling.
        className: cn("flex items-center justify-center min-h-screen w-full overflow-hidden", className), style: { backgroundColor, ...style }, children: _jsx("div", { ref: containerRef, 
